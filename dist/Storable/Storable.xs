...
#if !defined(PERL_VERSION) || __PERL_CORE_MAJOR__ == 5 && ( PERL_VERSION < 10 || (PERL_VERSION == 10 && PERL_SUBVERSION < 1))
        ...
        int network_order)
{
     ...
     */
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && PERL_VERSION >= 5)
...
#if __PERL_CORE_MAJOR__ == 5 && PERL_VERSION < 15
static void
cleanup_recursive_av(pTHX_ AV* av) {
    ...
    while (i >= 0) {
        if (arr[i]) {
#if __PERL_CORE_MAJOR__ == 5 && PERL_VERSION < 14
            arr[i] = NULL;
#else
            SvREFCNT_dec(arr[i]);
#endif
...
static int store_code(pTHX_ stcxt_t *cxt, CV *cv)
{
#if __PERL_CORE_MAJOR__ == 5 && PERL_VERSION < 6
    /*
...
#elif __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && ((PERL_VERSION==8) && (PERL_SUBVERSION >= 1) || (PERL_VERSION>8)))
...
static int get_regexp(pTHX_ stcxt_t *cxt, SV* sv, SV **re, SV **flags) {
    dSP;
    SV* rv;
#if __PERL_CORE_MAJOR__ == 5 && PERL_VERSION >= 12
...
static int sv_type(pTHX_ SV *sv)
{
    switch (SvTYPE(sv)) {
    case SVt_NULL:
#if __PERL_CORE_MAJOR__ == 5 && PERL_VERSION <= 10
    case SVt_IV:
#endif
    case SVt_NV:
        /*
    ...
    case SVt_PVCV:
        return svis_CODE;
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && PERL_VERSION > 8)
	/* case SVt_INVLIST: */
#endif
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && PERL_VERSION > 10)
    case SVt_REGEXP:
        return svis_REGEXP;
#endif
    default:
