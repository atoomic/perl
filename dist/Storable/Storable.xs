        ...
        int network_order)
{
     ...
     */
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ >= 5)
...
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 15
static void
cleanup_recursive_av(pTHX_ AV* av) {
    ...
    while (i >= 0) {
        if (arr[i]) {
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 14
            arr[i] = NULL;
#else
            SvREFCNT_dec(arr[i]);
#endif
...
static int get_regexp(pTHX_ stcxt_t *cxt, SV* sv, SV **re, SV **flags) {
    dSP;
    SV* rv;
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ >= 12
...
static int sv_type(pTHX_ SV *sv)
{
	...
	/* case SVt_INVLIST: */
#endif
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ > 10)
    case SVt_REGEXP:
        return svis_REGEXP;
#endif
    default:
