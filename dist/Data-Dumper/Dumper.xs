...
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ <= 6 /* Perl 5.6 and earlier */
...
#endif /* __PERL_CORE_MINOR__ <= 6 */

/* Perl 5.7 through part of 5.15 */
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ > 6 && __PERL_CORE_MINOR__ <= 15 && ! defined(utf8_to_uvchr_buf)
...
#endif /* __PERL_CORE_MINOR__ > 6 && <= 15 */
...
/* Changes in 5.7 series mean that now IOK is only set if scalar is
   ...
   complex test  */
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ <= 6
...
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 10
                sprintf(r, "\\x{%" UVxf "}", k);
                r += strlen(r);
...
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 20
	...
	AV *postav, const I32 level, SV *apad, Style *style)
{
...
#elif __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 11
	...
	if (realpack && !no_bless) {				/* we have a blessed ref */
	    ...
	    if (style->sortkeys) {
#if __PERL_CORE_MAJOR__ > 5 || (__PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ >= 8)
...
# endif
	...
	if (namelen) {
		...
		c += 4;
#if __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 7
		if (i == 6 || (i == 7 && c[6] == '\0'))
#else
		if (i == 6)
#endif
...
# if !defined(PL_vtbl_vstring) && __PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 17
	...
	PPCODE:
	{
		...
		if ((svp = hv_fetchs(hv, "sortkeys", FALSE))) {
                    ...
                    if (! SvTRUE(sv))
                        ...
                        style.sortkeys = sv;
                    else if (__PERL_CORE_MAJOR__ == 5 && __PERL_CORE_MINOR__ < 8)
