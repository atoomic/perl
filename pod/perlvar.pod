...
=item $OLD_PERL_CORE_MINOR

=item $]
X<$]> X<$OLD_PERL_CORE_MINOR>
...
The C<$OLD_PERL_CORE_MINOR> form was added in Perl v5.20.0 for historical
reasons but its use is discouraged. (If your reason to use C<$]> is to
run code on old perls then referring to it as C<$OLD_PERL_CORE_MINOR> would
be self-defeating.)
...
=item $PERL_CORE_MINOR

=item $^V
X<$^V> X<$PERL_CORE_MINOR>
...
=for apidoc hAmn|int|PERL_CORE_MINOR
=for apidoc hAmn|int|PERL_CORE_MAJOR
=for apidoc hAmn|int|PERL_CORE_RELEASE
