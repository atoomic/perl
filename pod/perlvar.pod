...
=item $OLD___PERL_CORE_MINOR__

=item $]
X<$]> X<$OLD___PERL_CORE_MINOR__>
...
The C<$OLD___PERL_CORE_MINOR__> form was added in Perl v5.20.0 for historical
reasons but its use is discouraged. (If your reason to use C<$]> is to
run code on old perls then referring to it as C<$OLD___PERL_CORE_MINOR__> would
be self-defeating.)
...
=item $__PERL_CORE_MINOR__

=item $^V
X<$^V> X<$__PERL_CORE_MINOR__>
...
=for apidoc hAmn|int|__PERL_CORE_MINOR__
=for apidoc hAmn|int|__PERL_CORE_MAJOR__
=for apidoc hAmn|int|__PERL_CORE_RELEASE__
