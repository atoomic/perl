...
The English module now sets $__PERL_CORE_MINOR__ to $^V (a string value) rather
...
$__PERL_CORE_MINOR__ now stands for C<$^V> (a string value) rather than for C<$]>
(a numeric value).
...
=item Format of $English::__PERL_CORE_MINOR__ is different
...
=item C<PATCHLEVEL> is now C<__PERL_CORE_MINOR__>

The cpp macros C<__PERL_CORE_MAJOR__>, C<__PERL_CORE_MINOR__>, and C<__PERL_CORE_RELEASE__>
are now available by default from perl.h, and reflect the base revision,
patchlevel, and subversion respectively.  C<__PERL_CORE_MAJOR__> had no
prior equivalent, while C<__PERL_CORE_MINOR__> and C<__PERL_CORE_RELEASE__> were
