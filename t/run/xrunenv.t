    ...
    for my $mode ( qw{NO RANDOM DETERMINISTIC} ) { # 0, 1 and 2 respectively
        ...
        if ($err=~/HASH_SEED = (0x[a-f0-9]+)/) {
            ...
            }
            {
                ...
                my($out2, $err2) = runperl_and_capture( { %base_opts, PERL_HASH_SEED => $updated_seed }, [ @print_keys ]);
                isnt ($out,$out2,"PERL_PERTURB_KEYS = $mode results in different order with a different key");
            }
        }
    }
}
