...
SKIP: {
    ...
    fresh_perl_is('no warnings q|uninitialized|; my $alpha = substr $^X, 0, 0; /\x{100}/i; /$alpha\x{100}/i || print q,ok,',
		  'ok', {switches => ["-T", "-l"]},
		  "matching a regexp is taint agnostic");

    fresh_perl_is('no warnings q|uninitialized|; my $alpha = substr $^X, 0, 0; /$alpha\x{100}/i || print q,ok,',
		  'ok', {switches => ["-T", "-l"]},
